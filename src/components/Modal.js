     ...
     if(e.keyCode == 13){
        ...
        foldersRemote.readFolder().then(data => {
          if(data.includes(this.state.value)){
            ...
            // createTheme(this.state.value, this.state.template).then(console.log('ok'));;
            this.setState({
               showInput: false,
               newFolder: true
            });
    ...
    return (
      ...
      <ModalBox  >
...

        {
          showInput
          ?
          <InputBox type='text' placeholder="Name of your Collection" value={value} onKeyDown={this.keyPress} onChange={this.handleChange}
          />
          :
          <SuccessMSG> You've started a new collection. <Name>{value}</Name>  </SuccessMSG>
